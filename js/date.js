  ...
  $.fn.date = function (options, Ycallback, Ncallback) {
    ...
    function bindButton() {
      ...
      $('#dateconfirm')
        ...
        .click(function () {
          var datestr =
            $('#yearwrapper ul li:eq(' + indexY + ')')
              .html()
              .substr(
                0,
                $('#yearwrapper ul li:eq(' + indexY + ')').html().length - 1
              ) +
            '-' +
            $('#monthwrapper ul li:eq(' + indexM + ')')
              .html()
              .substr(
                0,
                $('#monthwrapper ul li:eq(' + indexM + ')').html().length - 1
              ) +
            '-' +
            $('#daywrapper ul li:eq(' + Math.round(indexD) + ')')
              .html()
              .substr(
                0,
          ...
          if (datetime) {
            if (Math.round(indexS) === 1) {
              //下午
              $('#Hourwrapper ul li:eq(' + indexH + ')').html(
                parseInt(
                  $('#Hourwrapper ul li:eq(' + indexH + ')')
                    .html()
                    .substr(
                      0,
                      $('#Hourwrapper ul li:eq(' + indexH + ')').html().length -
                        1
                    )
                ) + 12
              )
            ...
            datestr +=
              ...
              $('#Hourwrapper ul li:eq(' + indexH + ')')
                ...
                .substr(
                  0,
                  $('#Minutewrapper ul li:eq(' + indexH + ')').html().length - 1
                ) +
              ':' +
              $('#Minutewrapper ul li:eq(' + indexI + ')')
                .html()
                .substr(
                  0,
                  $('#Minutewrapper ul li:eq(' + indexI + ')').html().length - 1
                )
            indexS = 0
          }
    ...
    function init_iScrll() {
      var strY = $('#yearwrapper ul li:eq(' + indexY + ')')
        .html()
        .substr(0, $('#yearwrapper ul li:eq(' + indexY + ')').html().length - 1)
      var strM = $('#monthwrapper ul li:eq(' + indexM + ')')
        .html()
        .substr(
          0,
          $('#monthwrapper ul li:eq(' + indexM + ')').html().length - 1
